...
Функция АдресХраненияПринадлежитФранчайзи(АдресХранения, ДатаДок)Экспорт //Грачев Р.А. 14.02.2018
	
	Запрос = Новый Запрос;
	Запрос.Текст = 
	"ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	УчастникиФранчайзи.Франчайзи
	|ИЗ
	|	РегистрСведений.УчастникиФранчайзи КАК УчастникиФранчайзи
	|ГДЕ
	|	УчастникиФранчайзи.АдресХранения = &АдресХранения
	|	И УчастникиФранчайзи.Использовать
	|	И УчастникиФранчайзи.ДатаПо >= &ДатаДок
	|	И НЕ УчастникиФранчайзи.ДатаПо = ДАТАВРЕМЯ(1, 1, 1)";
	
	Запрос.УстановитьПараметр("АдресХранения", АдресХранения);
	Запрос.УстановитьПараметр("ДатаДок", НачалоДня(ДатаДок));

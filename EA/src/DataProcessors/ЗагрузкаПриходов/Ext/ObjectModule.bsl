...
Процедура ЗагрузитьДокументы() Экспорт
	...
	Для каждого СтрокаШ из ДокументыКВыгрузке цикл
		...
		Для каждого строка из ТаблицаПриходов цикл
			...
			Если id_supplier = 0 тогда
				ТоварПоКодуПоставщика = справочники.Номенклатура.НайтиПоКоду(СокрЛП(КодПоставщика));
			Иначе
		...
		//Грачев
		Документ.ТипПрихода = Перечисления.ТипПрихода.Закупка;
		//Грачев

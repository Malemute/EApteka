...
//
Функция ВывестиРезультатОдногоЗапроса(ИмяЗапроса, РезЗапроса, Открыта, ВыводитьИдентификатор, ПорядокОбхода, Иерархия, КоличествоСтрок, МассивШириныКолонок, ИспользованиеЧередования, ЗаголовокДляРассылки = Ложь) Экспорт 
	...
	Заголовок			= ВывестиЗаголовокЗапроса(ИмяЗапроса, КоличествоСтрок,ЗаголовокДляРассылки);
	
	Если ЗаголовокДляРассылки тогда
		МакетОдногоЗапроса.Вывести(Заголовок);
	КонецЕсли;
	МакетОдногоЗапроса.НачатьАвтогруппировкуСтрок();
	
	Если НЕ ЗаголовокДляРассылки тогда
...
Функция ВывестиЗаголовокЗапроса(ИмяЗапроса,КоличествоСтрок,ЗаголовокДляРассылки = Ложь)
	Заголовок 	= Новый ТабличныйДокумент;
	
	МакетВывода = ПолучитьМакет("РезультатВыполненияЗапроса");
	Если ЗаголовокДляРассылки тогда
		ОбластьЗаголовок 	= МакетВывода.ПолучитьОбласть("ЗапросИмяРассылка");

...
Процедура РасчетСуммаНаличныеКасса()
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		СуммаВКассе = ВыборкаДетальныеЗаписи.СуммаОстаток;
	КонецЦикла;
	...
	//	FR_atol.GetRegister();
		Элементы.СуммаВКассе.Заголовок = "Сумма в кассе: " + Формат(СуммаВКассе, "ЧДЦ=2") + " руб.";	
	//КонецЕсли;
	//<<Хортюк С.Б. 20.02.2018
...
Процедура ПровестиДокументЧек(ЧекСсылка,ТекНомерКПК)
	...
	ДокументЧек.УстановитьНовыйНомер();
...
Процедура Внесение(Команда)
	Если Число(ЭтаФорма.Элементы.СуммаВнесения.ТекстРедактирования)>0 Тогда
		...
		ОповещениеПриЗавершении = Новый ОписаниеОповещения("ФУ_ВнесениеДенегЗавершение", ЭтотОбъект);
...
Процедура Изъятие(Команда)
	...
	Если СуммаИнкассации > 0 Тогда
		...
		ОповещениеПриЗавершении = Новый ОписаниеОповещения("ФУ_ВыемкаДенегЗавершение", ЭтотОбъект);
		МенеджерОборудованияКлиент.НачатьИнкассациюНаФискальномУстройстве(ОповещениеПриЗавершении, УникальныйИдентификатор, ПараметрыОперации, ФискальноеУстройство);
...
Функция НайтиЧекПоКПКНаСервере(НомерКПК)
	...
	Иначе
		...
		Пока ВыборкаЧеки.Следующий() цикл
			...
			Объект.СсылочныйНомер		= ВыборкаЧеки.rrn;
			КодАвторизации		= ВыборкаЧеки.authcode;
...
&НаКлиенте
Процедура ФУ_ВнесениеДенегЗавершение(РезультатВыполнения, Параметры) Экспорт
	...
	Если РезультатВыполнения.Результат Тогда
		ТекстСообщения = НСтр("ru='Внесены деньги на сумму: '" + СуммаИнкассации);
		ФУ_ОформитьФискальныйРезультат(РезультатВыполнения, ТекстСообщения);
		
		СоздатьВнесениеДенег();
	Иначе
		ТекстСообщения = РезультатВыполнения.ОписаниеОшибки;
		// Пишем лог
...
&НаСервере
Процедура СоздатьВнесениеДенег()
	
	НовДок = Документы.ВнесениеДенежныхСредствВКассуККМ.СоздатьДокумент();
	НовДок.Дата = ТекущаяДата();
	НовДок.Организация = ККМФирма;
	НовДок.Ответственный = Объект.Сотрудник;
	НовДок.КассаККМ = ККМСсылка;
	НовДок.СуммаДокумента = СуммаИнкассации;
	НовДок.Записать(РежимЗаписиДокумента.Проведение);
	
КонецПроцедуры  // СоздатьВнесениеДенег()

&НаКлиенте
Процедура ФУ_ВыемкаДенегЗавершение(РезультатВыполнения, Параметры) Экспорт
	...
	Если РезультатВыполнения.Результат Тогда
		ТекстСообщения = НСтр("ru='Изъяты деньги на сумму: '" + СуммаИнкассации);
		ФУ_ОформитьФискальныйРезультат(РезультатВыполнения, ТекстСообщения);
...
&НаСервере
Процедура СоздатьВыемкуДенег()
	
	НовДок = Документы.ВыемкаДенежныхСредствИзКассыККМ.СоздатьДокумент();
	НовДок.Дата = ТекущаяДата();
	НовДок.Организация = ККМФирма;
	НовДок.Ответственный = Объект.Сотрудник;
	НовДок.Магазин = Объект.ТекущийСклад;
	НовДок.КассаККМ = ККМСсылка;
	НовДок.СуммаДокумента = СуммаИнкассации;
	НовДок.Записать(РежимЗаписиДокумента.Проведение);
	
КонецПроцедуры  // СоздатьВыемкуДенег()

&НаКлиенте
Процедура ФУ_ОформитьФискальныйРезультат(Знач РезультатВыполнения, ТекстСообщения)
	
	Перем СайтПроверкиОФД, СтатусСмены, ФискальныйПризнак;

...
Процедура ПриОткрытии(Отказ)
	...
	//>>Хортюк С.Б. 19.02.2018 При закрытии смены ККМ снимать z-отчет по эквайрингу
...
Процедура ВыполнитьЗакрытиеСмены()
	...
	//ПараметрыОперации.Кассир = ?(Не СтандартнаяОбработка, Кассир, НСтр("ru='Администратор'")); 
	//
	//ОповещениеПриЗавершении = Новый ОписаниеОповещения("ЗакрытиеСменыЗавершение", ЭтотОбъект);
...
Процедура ЭТ_ВыполнитьОперациюЗавершение(РезультатВыполнения, Параметры) Экспорт
	...
	СуммаИнкассации = РасчетСуммаНаличныеКасса();
	
	Если СуммаИнкассации > 0 Тогда

		ФискальноеУстройство = Кассы[0].ПодключаемоеОборудование;
...
Процедура ФУ_ВыемкаДенегЗавершение(РезультатВыполнения, Параметры) Экспорт
	...
	Если РезультатВыполнения.Результат Тогда
		...
		Если СуммаИнкассации <> 0 Тогда
			СоздатьВыемкуДенег();
			ЗаголовокИнформации = "Изъятие денег";
			...
			ТекстПереходящегоОстатка = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстПереходящегоОстатка, 
																					 Формат(СуммаИнкассации, "ЧЦ=15; ЧДЦ=2; ЧРГ=' '; ЧН=0,00; ЧГ=3,0"));
			ОбщегоНазначенияКлиент.ВывестиИнформациюДляРМКУправляемой(ЗаголовокИнформации, ТекстПереходящегоОстатка);
		КонецЕсли;
	Иначе
		ТекстСообщения = РезультатВыполнения.ОписаниеОшибки;
...
&НаКлиенте
Процедура ЗакрытиеСменыЗавершение(РезультатВыполнения, Параметры) Экспорт
...
Функция РасчетСуммаНаличныеКасса()
	...
	Запрос.УстановитьПараметр("КассаККМ", Кассы[0].Касса);
...
КонецФункции // РасчетСуммаНаличныеКасса()

...
Процедура УстановитьСсылкуДляОбъекта(СтрокаСтекаНезаписанныхОбъектов)
	...
	//---%%% Тарасков - Начало
	// Необходимо предусмотреть тот факт, что метод УстановитьСсылкуНового может быть вызван для ОбъектДляЗаписи в обработчких правил обмена
	//НовыйУникальныйИдентификатор = Новый УникальныйИдентификатор;
	//НоваяСсылка = Менеджер.ПолучитьСсылку(НовыйУникальныйИдентификатор);
	//	
	//ОбъектДляЗаписи.УстановитьСсылкуНового(НоваяСсылка);
	Если Не ОбъектДляЗаписи.Ссылка.Пустая() Тогда
		НоваяСсылка = ОбъектДляЗаписи.Ссылка;
	Иначе
		
		НоваяСсылка = ОбъектДляЗаписи.ПолучитьСсылкуНового();

...
Процедура ЗагрузкаМаршрутовИЗаказов() Экспорт
	...
	ТаблицаЗаказов = ОбменАлгоритмы77.ВыполнитьСверкуПоМетаданному(ОписаниеОбъекта77,1000,,,МассивЗаказов,Истина,,Перечисления.ТипКомплектацииЗаказа.СборкаНаСкладеВПакеты_РаскладкаПакетовВАптеке);
...
Процедура ЗагрузитьСамовывозИзКоробки() Экспорт
	...
	ОписаниеОбъекта77 = ОбменАлгоритмы77.ПолучитьОписаниеТаблицыВБазе77("Документ","Заказ");
	ТаблицаЗаказов = ОбменАлгоритмы77.ВыполнитьСверкуПоМетаданному(ОписаниеОбъекта77,100,,,МассивЗаказов,Истина,,Перечисления.ТипКомплектацииЗаказа.СборкаНаСкладеВКоробку_КомплектацияВАптеке); 
...
Процедура ВыгрузитьПеремещенияВ77() Экспорт
	...
	Запрос.УстановитьПараметр("ТекАХ", ПараметрыСеанса.ТекущийАдресХранения);
	МассивРезультатов = Запрос.ВыполнитьПакет();
...
Процедура УстановитьСтатусыЗаказовПоРегиструСведений(ЕстьОшибки)
	...
	|	Документ.Заказ КАК Заказ
	|ГДЕ
	|	НЕ Заказ.СтатусыОбработкиЗаказа В (&КонечныеСтатусыОбработкиЗаказа)
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	Ссылка
	|;

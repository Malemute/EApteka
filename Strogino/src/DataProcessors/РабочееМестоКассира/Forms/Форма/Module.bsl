...
Функция ЗаписатьДокументЧек(ЗаводскойНомер,НомерЧека,НомерСмены,rrn,authcode,НомерКарты)
	...
	ДокументЧек.ТипОплаты = ?(ЭтаФорма.ОплатаКартой,
								Перечисления.ТипОплаты.Безнал,
								Перечисления.ТипОплаты.Наличные);
	ДокументЧек.КПК = "";
...
//************************************************** ЕМ 	
	...
	Для Каждого СтрокаТЧ Из Объект.Чек Цикл
		...
		СтрокаПозицииЧека.СтавкаНДС    = РегистрыСведений.СтавкиНДСПоНоменклатуре.ВернутьСтавкуНДСПоНоменклатуре(СтрокаТЧ.Товар, ТекущаяДата(), Ложь);

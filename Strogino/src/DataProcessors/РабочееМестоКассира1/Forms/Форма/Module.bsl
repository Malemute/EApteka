...
Функция НайтиТоварыСервере(ПоШК)
	...
	|		ПО (Номенклатура.ФонетическийКодСтрока ПОДОБНО втСловаСЗаполненнымФонетическимКодом.mLikeWordМаска)
	|ГДЕ
	|	НЕ &ПоМНН
	|
	|СГРУППИРОВАТЬ ПО
...
Процедура ПоискТовараВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если ТекущиеДанные <> Неопределено тогда
		...
		ПоказатьНаДисплее(ТекущиеДанные.Товар.Наименование,СделатьСтрокуЦен(ТекущиеДанные.Цена,Объект.Чек.Итог("Всего")));
	КонецЕсли;
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	//горячия клавиши
	Элементы.ПробитьЧек.СочетаниеКлавиш =новый СочетаниеКлавиш(Клавиша.F9,Ложь,Ложь,Ложь); //F9	
	...
	Самовывозы.Параметры.УстановитьЗначениеПараметра("АдресХранения",ПараметрыСеанса.ТекущийАдресХранения);
...
Функция ИнициализироватьФорму()
	...
	Элементы.ТекстНормерККМ.Заголовок = Строка(НомерККМ)+"  /  "+ИмяКомпьютера();
...
&НаСервере
Процедура ЗаполнитьДокументЧекПоБезналу(ЧекСсылка,rrn,authcode,НомерКарты)
...
Процедура ПробитьЧек(Команда)
	Перем card_id, rrn, authcode, Slip, MustCommit,VoicePay,НомерЧека,ЗаводскойНомер,НомерСмены;
	...
	Если НЕ ЗаполнитьЧекККМФискальныйРегистратор(НомерЧека,ЗаводскойНомер,НомерСмены) тогда
		...
		ПоказатьПредупреждение(,"Не удалось зпаполнить чек!!!");
		Возврат;
	КонецЕсли;
	
	Попытка
		ЧекСсылка = ЗаписатьДокументЧек(ЗаводскойНомер,НомерЧека,НомерСмены,rrn,authcode,card_id);
	Исключение
	...
	Если не ЗначениеЗаполнено(ЧекСсылка) тогда
		Возврат;	
	КонецЕсли;	
	...
	Если ЭтаФорма.ОплатаКартой Тогда
		...
		Если TerminalDriverID = "AddIn.IngenicoDriver1C" Тогда
			Если Не IngenicoПодключить() Тогда
				Возврат;
			КонецЕсли;
			
			Если не ЭтаФорма.ВозвратПродажи Тогда		//Продажи
				
				Если Не IngenicoОплатитьПлатежнойКартой(card_id, Объект.Чек.Итог("Всего"), НомерЧека, rrn, authcode, Slip) Тогда
					ФискальныйРегистраторАннулировать(FR, Slip);
					IngenicoОтключить();
					Возврат;
				КонецЕсли;
			...
			ИначеЕсли ЭтаФорма.ВозвратПродажи Тогда	//Возврат
				
				Если Не IngenicoОтменитьПлатежПоПлатежнойКарте(card_id, Объект.Чек.Итог("Всего"), Объект.Возврат_НомерЧека, Объект.rrn, Объект.КодАвторизации, Slip) Тогда //Попытка отмены платежа
					Если Не IngenicoВернутьПлатежПоПлатежнойКарте(card_id, Объект.Чек.Итог("Всего"), Объект.Возврат_НомерЧека, Объект.rrn, Объект.КодАвторизации, Slip) Тогда //Попытка возврата платежа
						ФискальныйРегистраторАннулировать(FR, Slip);
						IngenicoОтключить();
						Возврат;
					КонецЕсли;
			...
			Иначе
				ФискальныйРегистраторАннулировать(FR, Slip);
				IngenicoОтключить();
				Сообщить("Тип документа не является продажей либо возвратом. Оплата невозможна.");
				Возврат;
			КонецЕсли;
		...
		ИначеЕсли TerminalDriverID = "AddIn.UCS_EFTPOS" Тогда
			
			Если ЭтаФорма.ВозвратПродажи Тогда
				MustCommit = Ложь;
				VoicePay=Ложь;
				Если Не ТерминалОплаты.SmartPay("", card_id, Объект.Чек.Итог("Всего"), rrn, authcode, Slip, VoicePay, MustCommit) Тогда
					GetLastError();
					FR.Password=30;
			...
			Иначе
				MustCommit=Ложь;
				VoicePay=Ложь;
				Если Не ТерминалОплаты.SmartPay("", card_id, -1*Объект.Чек.Итог("Всего"), Объект.rrn, Объект.КодАвторизации, Slip, VoicePay, MustCommit) Тогда //Попытка отмены платежа
					Если Не ТерминалОплаты.SmartPay("", card_id, -1*Объект.Чек.Итог("Всего"), rrn, authcode, Slip, VoicePay, MustCommit) Тогда //если не получилось, то возврат платежа по карте
						GetLastError();
						FR.Password=30;
						FR.SysAdminCancelCheck();
						PaperStatus();
		...
		Попытка
			ЗаполнитьДокументЧекПоБезналу(ЧекСсылка,rrn,authcode,card_id);
		Исключение
			Сообщить("Не удалось");
		КонецПопытки;
		
	Иначе
		Если Объект.СуммаПокупателя>0 И не ЭтаФорма.ВозвратПродажи Тогда
			Если Объект.СуммаПокупателя>Объект.Чек.Итог("Всего") Тогда
				Если Объект.ТипККМ = ПредопределенноеЗначение("Перечисление.ТипККМ.ШТРИХ_М") тогда
					FR.Summ1=Объект.СуммаПокупателя;
					FR.Summ4 = 0;
					//Иначе
					//	FR_atol.TypeClose = 0;
				КонецЕсли;
				ПоказатьНаДисплее("ВНЕСЕНО: "+Формат(Объект.СуммаПокупателя,"ЧРД=.; ЧРГ=' '; ЧН="),"СДАЧА: "+Формат(Объект.СуммаПокупателя-Объект.Чек.Итог("Сумма"),"ЧРД=.; ЧРГ=' '; ЧН="));
				Сообщить("ВНЕСЕНО: "+Формат(Объект.СуммаПокупателя,"ЧРД=.; ЧРГ=' '; ЧН=") +Символы.ПС +"СДАЧА: "+Формат(Объект.СуммаПокупателя-Объект.Чек.Итог("Сумма"),"ЧРД=.; ЧРГ=' '; ЧН="));
			КонецЕсли;
		КонецЕсли;	
	КонецЕсли;
	...
	Если Объект.ТипККМ = ПредопределенноеЗначение("Перечисление.ТипККМ.ШТРИХ_М") тогда
		...
		FR.Summ4=0;	
		//КонецЕсли;
		FR.GetEKLZCode1Report();
		ТекНомерКПК=Формат(FR.LastKPKNumber,"ЧРД=.; ЧН=; ЧГ="); 
	Иначе
		FR_atol.НомерРегистра=51;
		FR_atol.ПолучитьРегистр();
		ТекНомерКПК = FR_atol.DocNumber;
		FR_atol.TypeClose = 0;
	КонецЕсли;                   
	
	FR.Disconnect();
	
	ПровестиДокументЧек(ЧекСсылка,ТекНомерКПК);
	...
	Если НЕ ЭтаФорма.ВозвратПродажи тогда
		Если ПодключениеСМС тогда
			...
			Если Телефон <> неопределено тогда
				ОтправитьСМСПромоВФоне(ЧекСсылка,Телефон,Объект.СотрудникСсылка);	
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
...
&НаКлиенте
Процедура ОтчетСГашением(Команда)
	Перем Slip;
	Если Вопрос("Вы уверены что хотите снять Z-отчет?",РежимДиалогаВопрос.ДаНет)=КодВозвратаДиалога.Да Тогда
		Если Объект.ТипККМ = ПредопределенноеЗначение("Перечисление.ТипККМ.ШТРИХ_М") Тогда 
			FR.Password=30;
			FR.GetECRStatus();
			...
			Если TerminalDriverID = "AddIn.IngenicoDriver1C" Тогда
				...
				ОписаниеОшибки = "";
				Если Не IngenicoЭквайринговыйТерминал.ИтогиДняПоКартам(TerminalID, Slip) Тогда
					КодОшибки = IngenicoЭквайринговыйТерминал.ПолучитьОшибку(ОписаниеОшибки);
					Сообщить("Код: " + СокрЛП(Строка(КодОшибки)) + " " + ОписаниеОшибки, СтатусСообщения.ОченьВажное);
					IngenicoОтключить();
					Возврат;
				Иначе
					ВсегоСтрок = СтрЧислоСтрок(Slip);
					...
					FR.CutCheck();
					Сообщить("Эквайринговый терминал: Произведена сверка итогов дня. Должна распечататься квитанция закрытия дня (отчёт закрытия смены).");
				КонецЕсли;
			...
			ИначеЕсли TerminalDriverID = "AddIn.UCS_EFTPOS" Тогда
				...
				Если Объект.ТерминалОткрыт Тогда
					Если Не ТерминалОплаты.Settlement("", Slip) Тогда // Закрытие дня по картам
						GetLastError();
					Иначе
						ВсегоСтрок = СтрЧислоСтрок(Slip);
						Для Сч = 1 По ВсегоСтрок Цикл
		...
		Если МассивСтрок <> Неопределено и МассивСтрок.Количество() > 0 Тогда
			...
			Иначе
				Для ИндексСтрока = 0 По МассивСтрок.Количество() - 1  Цикл
					...
					ПоискТовара = Лев(ЭлементМассива.Товар,96);
					
					FR_atol.Password=30;
					FR_atol.Caption= ПоискТовара;
					FR_atol.Alignment = 0;
					FR_atol.TextWrap = 2;
					FR_atol.RecFont = 2;
					FR_atol.PrintField();
...
&НаКлиенте
Функция IngenicoОплатитьПлатежнойКартой(НомерКарты, Сумма, НомерЧека, КодRRN, КодАвторизации, ТекстКвитанции)
	Перем ОписаниеОшибки;
	
	//Параметры
	//TerminalID	- должна уже быть	// (DeviceID) STRING  [IN] Идентификатор устройства
	//НомерКарты	- параметр			// (CardNumber) STRING [IN/OUT] Данные карты. См. раздел «Передача данных карты»
	//Сумма			- параметр			// (Amount) DOUBLE [IN] Сумма операции по карте 
	//НомерЧека		- параметр			// (ReceiptNumber) STRING [IN/OUT] Номер чека
	//КодRRN		- параметр			// (RRNCode) STRING [OUT] Уникальный код транзакции RRN
	//КодАвторизации- параметр			// (AuthorizationCode) STRING [OUT] Код авторизации транзакции
	//ТекстКвитанции- параметр			// (Slip) STRING [OUT] Текст квитанции, сформированный Эквайринговым ПО
	
	IngenicoЭквайринговыйТерминал.УстановитьПараметр("TerminalID", TerminalID);
	
	ОплатаУспешна = IngenicoЭквайринговыйТерминал.ОплатитьПлатежнойКартой(
	TerminalID
	,НомерКарты
	,Сумма
	,НомерЧека
	...
	//Сообщить о результате выполнения операции
	ОписаниеОшибки = "";
	Если ОплатаУспешна Тогда
		Сообщить("Оплата проведена успешно.");
	Иначе
		КодОшибки = IngenicoЭквайринговыйТерминал.ПолучитьОшибку(ОписаниеОшибки);
		Сообщить("Код: " + СокрЛП(Строка(КодОшибки)) + " " + ОписаниеОшибки, СтатусСообщения.ОченьВажное);
	КонецЕсли;
	
	Возврат ОплатаУспешна;
КонецФункции

&НаКлиенте
Функция IngenicoПодключить()
	Перем ИДУстройства, РезультатПодключения, ОписаниеОшибки;
	ОписаниеОшибки = "";
	
	РезультатПодключения = Ложь;
	Попытка
		IngenicoЭквайринговыйТерминал.УстановитьПараметр("TerminalID", TerminalID);
		РезультатПодключения = IngenicoЭквайринговыйТерминал.Подключить(ИДУстройства);
		
		Если ЗначениеЗаполнено(РезультатПодключения) Тогда
			Если ТипЗнч(РезультатПодключения) = Тип("Булево") Тогда
				Если Не РезультатПодключения Тогда
					Сообщить("Несоответствие ЭТ: IngenicoЭквайринговыйТерминал.Подключить(ИДУстройства)");
					КодОшибки = IngenicoЭквайринговыйТерминал.ПолучитьОшибку(ОписаниеОшибки);
					Сообщить("Код: " + СокрЛП(Строка(КодОшибки)) + " " + ОписаниеОшибки, СтатусСообщения.ОченьВажное);
				Иначе
			...
			КонецЕсли;
		Иначе
			Сообщить("Несоответствие ЭТ: Результат подключения отсутствует.");
			КодОшибки = IngenicoЭквайринговыйТерминал.ПолучитьОшибку(ОписаниеОшибки);
			Сообщить("Код: " + СокрЛП(Строка(КодОшибки)) + " " + ОписаниеОшибки, СтатусСообщения.ОченьВажное);
		КонецЕсли;
	Исключение
		Сообщить("Несоответствие ЭТ: Сбой подключения эквайрингового терминала.");
		КодОшибки = IngenicoЭквайринговыйТерминал.ПолучитьОшибку(ОписаниеОшибки);
		Сообщить("Код: " + СокрЛП(Строка(КодОшибки)) + " " + ОписаниеОшибки, СтатусСообщения.ОченьВажное);
	КонецПопытки;
...
&НаКлиенте
Процедура IngenicoОтключить()
	Перем ИДУстройства;
	
	Попытка
		IngenicoЭквайринговыйТерминал.УстановитьПараметр("TerminalID", TerminalID);
		РезультатОтключения = IngenicoЭквайринговыйТерминал.Отключить(ИДУстройства);
	Исключение
		Сообщить("Несоответствие ЭТ: Сбой отключения эквайрингового терминала.")
	КонецПопытки;
	
КонецПроцедуры

&НаКлиенте
Функция IngenicoОтменитьПлатежПоПлатежнойКарте(НомерКарты, Сумма, НомерЧека, КодRRN, КодАвторизации, ТекстКвитанции)
	Перем ОписаниеОшибки, ОперацияУспешна;
	...
	ОперацияУспешна = IngenicoЭквайринговыйТерминал.ОтменитьПлатежПоПлатежнойКарте(
	TerminalID
	,НомерКарты
	,Сумма
	,НомерЧека
	...
	Если ОперацияУспешна Тогда
		Сообщить("Отмена платежа по платёжной карте произведена успешно.");
	Иначе
		КодОшибки = IngenicoЭквайринговыйТерминал.ПолучитьОшибку(ОписаниеОшибки);
		Сообщить("Код: " + СокрЛП(Строка(КодОшибки)) + " " + ОписаниеОшибки, СтатусСообщения.ОченьВажное);
	КонецЕсли;
	
	Возврат ОперацияУспешна;
КонецФункции

&НаКлиенте
Функция IngenicoВернутьПлатежПоПлатежнойКарте(НомерКарты, Сумма, НомерЧека, КодRRN, КодАвторизации, ТекстКвитанции)
	Перем ОписаниеОшибки, ОперацияУспешна;
	...
	ОперацияУспешна = IngenicoЭквайринговыйТерминал.ВернутьПлатежПоПлатежнойКарте(
	TerminalID
	,НомерКарты
	,Сумма
	,НомерЧека
...
Процедура ФискальныйРегистраторАннулировать(fr = неопределено, slip= неопределено)
	...
	ПечатьКвитанцииSlip(FR, Slip);
	//Если ЭтаФорма.ОплатаКартой Тогда
	//	ПереходВТипОплатыНаличными(); //ТОDO: По результатам совещания от 03.11.16 17:30 - Валявко А.М. предложил добавить команду ПОВТОР
	//КонецЕсли;
...
&НаКлиенте
Функция ЗаполнитьЧекККМФискальныйРегистратор(НомерЧека,ЗаводскойНомер,НомерСмены)
	Если Объект.ТипККМ = ПредопределенноеЗначение("Перечисление.ТипККМ.ШТРИХ_М") тогда
		FR.НомерЛУ=1;
		FR.УстановитьАктивноеЛУ();
		FR.Connect();      
		Если FR.ResultCode<>0 Тогда
			Сообщить(СокрЛП(FR.ResultCodeDescription)+"!");
			Возврат Ложь ;
		КонецЕсли;      
		PaperStatus();
		FR.GetECRStatus();
		Если FR.ECRMode=8 Тогда 
			FR.Password=30;
			FR.SysAdminCancelCheck();
			PaperStatus();
		КонецЕсли;
		
		Если FR.ECRMode = 3 Then
			FR.Password = 30;
			FR.PrintReportWithCleaning();
			PaperStatus();
		КонецЕсли;
		
		Если СокрЛП(Объект.СотрудникФИО)<>"" Тогда
			НапечататьСтроку("Кассир "+СокрЛП(Объект.СотрудникФИО));   
		КонецЕсли;
		
		
		Для Каждого СтрокаТЧ Из Объект.Чек Цикл
			FR.Password=30;
			FR.StringForPrinting=СокрЛП(СтрокаТЧ.Товар); 
			FR.Quantity=СтрокаТЧ.Количество;
			FR.Price=СтрокаТЧ.Цена;        
			FR.Department=1;   
			...
			Пока Код <> 0 Цикл		
				...
				ИначеЕсли Код=88 Тогда
					FR.ContinuePrint();
				Иначе                   
					Предупреждение("1111 Ошибка кассы: "+FR.ResultCodeDescription+"! Код ошибки "+Строка(Код),5);
				КонецЕсли;
				PaperStatus();
				FR.Password = 30;
				Если НЕ ЭтаФорма.ВозвратПродажи Тогда
		...
		FR.Summ1=0;
		//
		//Если Объект.Чек.Итог("Скидка")>0 Тогда		
		//	FR.Password=30;
		//	PaperStatus();
		//	FR.StringForPrinting=ТекстСкидки;
		//	FR.Password=30;         
		//	FR.Summ1=Объект.ЧекОтображение.Итог("Скидка");    
		//	FR.Discount(); 
		//	PaperStatus();
		//КонецЕсли;
		...
		НомерЧека=Число(FR.OpenDocumentNumber)+1;
		ЗаводскойНомер=Формат(Число(FR.SerialNumber),"ЧРД=.; ЧН=; ЧГ=");
		НомерСмены=FR.SessionNumber;
		
	Иначе
		
		СоответствиеСтавокНДСиККМ = Новый Соответствие;
		СоответствиеСтавокНДСиККМ.Вставить(0,1);
		СоответствиеСтавокНДСиККМ.Вставить(10,2);
		СоответствиеСтавокНДСиККМ.Вставить(18,3);
		
		
		FR_atol.Password = 30; 
		FR_atol.Mode = 1; 
		КодОшибки = FR_atol.NewDocument();
		СчетчикПопыток = 0;
		Пока КодОшибки <> 0 цикл
			Если КодОшибки = -3807 тогда
				Предупреждение("Ошибка кассы нет бумаги! Вставьте бумагу.",5);	
		...
		Для каждого СтрокаТЧ из Объект.Чек цикл
			МассивСтавок.Добавить(СтрокаТЧ.Товар);
		КонецЦикла;
		СоответствиеСтавокНДС = ПолучитьСтавкиНДС(МассивСтавок);
		
		Для Каждого СтрокаТЧ Из Объект.Чек Цикл
			Если СтрокаТЧ.Количество = 0 или СтрокаТЧ.Цена = 0 тогда
				Продолжить;
			КонецЕсли;
			ПробитьПоТаблице = Ложь;
			ТабСкидок.Очистить();
			Если СтрокаТЧ.Скидка > 0 и СтрокаТЧ.Цена > 0 тогда
				СкидкаНаПозициюБезОкр = СтрокаТЧ.Скидка/СтрокаТЧ.Количество;
				СкидкаНаПозициюСОкр = Окр(СкидкаНаПозициюБезОкр,2);
				Если СкидкаНаПозициюБезОкр <> СкидкаНаПозициюСОкр тогда
					ПробитьПоТаблице = Истина;
					Строка = ТабСкидок.Добавить();
					Строка.Количество = СтрокаТЧ.Количество-1;
					Строка.Скидка = СкидкаНаПозициюСОкр*(СтрокаТЧ.Количество-1);
					
					Строка = ТабСкидок.Добавить();
					Строка.Количество = 1;
					Строка.Скидка = СтрокаТЧ.Скидка - СкидкаНаПозициюСОкр*(СтрокаТЧ.Количество-1);					
				КонецЕсли;
			КонецЕсли;
			Если ПробитьПоТаблице тогда
				Для каждого строка из ТабСкидок цикл
					FR_atol.Name = СокрЛП(СтрокаТЧ.Товар); 
					FR_atol.Price = СтрокаТЧ.Цена; 
					FR_atol.Quantity = строка.Количество; 
					FR_atol.Department=1;
					FR_atol.DiscountType = 0;
					FR_atol.DiscountValue = строка.Скидка;
					
					СтавкаНДС = СоответствиеСтавокНДС.Получить(СтрокаТЧ.Товар);
					СтавкаНДСККМ = Неопределено;
					Если СтавкаНДС <> неопределено тогда
						СтавкаНДСККМ = СоответствиеСтавокНДСиККМ.Получить(СтавкаНДС);
					КонецЕсли;
					
					Если СтавкаНДСККМ <> неопределено тогда
						FR_atol.TaxTypeNumber = СтавкаНДСККМ;
					Иначе
						FR_atol.TaxTypeNumber = 1;
					КонецЕсли;
					
					Если не ЭтаФорма.ВозвратПродажи Тогда
						КодОшибки = FR_atol.Registration();
					Иначе
						КодОшибки = FR_atol.Return();
					КонецЕсли;
			...
			Иначе
				FR_atol.Name = СокрЛП(СтрокаТЧ.Товар); 
				FR_atol.Price = СтрокаТЧ.Цена; 
				FR_atol.Quantity = СтрокаТЧ.Количество; 
				FR_atol.Department=1;
				FR_atol.DiscountType = 0;
				FR_atol.DiscountValue = СтрокаТЧ.Скидка;
				СтавкаНДС = СоответствиеСтавокНДС.Получить(СтрокаТЧ.Товар);
				СтавкаНДСККМ = Неопределено;
				Если СтавкаНДС <> неопределено тогда
					СтавкаНДСККМ = СоответствиеСтавокНДСиККМ.Получить(СтавкаНДС);
		...
		НомерЧека=Число(FR_atol.CheckNumber);
		ЗаводскойНомер=Прав(FR_atol.СерийныйНомер,6);
		НомерСмены=FR_atol.Session+1;
	КонецЕсли;
	Возврат Истина;
КонецФункции
...
Процедура ДобавитьТоварВПечатьЧеков(ШК)
	...
	ПоискТовара = ТоварНайтиПоШК(ШК);
	Если ЗначениеЗаполнено(ПоискТовара) тогда
		Если ТоварыДляПечатиЦенников.НайтиСтроки(Новый Структура("Товар",ПоискТовара)).Количество() = 0 Тогда
			СтрокаПЦ = ТоварыДляПечатиЦенников.Добавить();	
			СтрокаПЦ.Товар = ПоискТовара;
		КонецЕсли;
	КонецЕсли;
...
Процедура ПечатьЗСЯЗаказаНаККМ(Команда)
	...
	Для каждого строка из МассивСтрокПечати цикл
		НапечататьСтроку(строка,2);		
	КонецЦикла;
